  ...
  return (
    <View className="mt-4">
      <View className="mb-2 flex-row items-center justify-between">
        <Text className="text-lg font-bold text-[#1f2a44]">Popular Brands</Text>
      ...
      <View className="flex-row flex-wrap justify-between gap-y-2">
        {brands.slice(0, 6).map((brand) => (
          <View key={brand._id} className="items-center rounded-2xl border border-[#dde6ff] bg-white py-2" style={{ width: "31%" }}>
            <View className="h-9 w-9 items-center justify-center overflow-hidden rounded-full bg-[#eaf1ff]">
              {brand.image ? (
