...
const BabyTravelSection = () => {
    ...
    const loadProducts = async () => {
      try {
        ...
        const response: ProductResponse =
          await fetchData<ProductResponse>("/products");
        setProducts(response?.products?.slice(0, 8) || []);
        console.log(response?.products?.slice(0, 8) || []);
      } catch (error) {
        console.log("Failed to fetch baby travel products!", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="py-12 p-5 mt-5 rounded-md border bg-babyshopWhite">
      <div className="flex items-center justify-between mb-8">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <Plane className="h-5 w-5 text-babyshopSky" />
            <Badge
              variant={"outline"}
          ...
          <p className="text-gray-600">
            Everything you need for safe and comfortable travel with your little
            one.
          </p>
        </div>
        <Link href={"/shop?category=item"}>
          <Button
            variant={"outline"}
            className="hidden md:flex items-center gap-2 hover:bg-babyshopSky hover:text-white hover:border-babyshopSky transition-colors border-black"
          >
            Shop All Items →
          </Button>
        </Link>
      ...
      {/* Promotion Banner */}
      <div className="bg-gradient-to-r from-babyshopSky to-blue-600 p-8 rounded-2xl text-white">
      ...
      <div className="mt-8 text-center md:hidden text-black">
        <Link href="/shop?category=travel">
          <Button className="w-full bg-babyshopSky hover:bg-babyshopSky/90">
